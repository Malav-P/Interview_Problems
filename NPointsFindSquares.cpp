    ...
    for (size_t m = 0; m < N ; m++)
    {
        ...
        for (size_t n = 0; n < m; n++)
        {
            ...
            if (dist == distances[m][n])
            {
                ...
                {
                    // if dot product is 0
                    Point diff_ij = diff(pts[i], pts[j]);
                    Point diff_mn = diff(pts[m], pts[n]);

                    if (dot(diff_ij, diff_mn) == 0.0)
                    {
                        ans += 1;
                    }
                }
...
size_t num_Squares(vector<int> x, vector<int> y)
{
...
int main() {
    ...
    std::cout << "Number of squares: " << num_Squares(x,y);
}
